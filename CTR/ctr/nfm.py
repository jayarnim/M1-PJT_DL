...
class InteractionEffect(nn.Module):
    def __init__(self, embed_dim, hidden, dropout, field_dim, field_type):
        super().__init__()
        self.embed_dim = embed_dim
        self.hidden = hidden
        self.dropout = dropout
    ...
    def _assert_arg_error(self):
        CONDITION = (self.hidden[0]==self.embed_dim*len(self.field_dim))
        ERROR_MESSAGE = f"First hidden units must match input size: {self.embed_dim*len(self.field_dim)}"
