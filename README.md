...
종합하자면, 쌍선형 관계 구조를 가정하는 모형은 연산 과정이 명시적(Explicit)이라는 점에서 분석 목적에는 적합하지만, 가정의 제약으로 인하여 표현력에 한계가 있어 예측력을 기대하기 어렵다는 것이 일반적인 통념이다. 그러나 귀납적 편향에 상응하는 변수 설계가 수반되는 경우, 외부 교란에도 강건한 예측력을 확보할 수 있음을 확인하였다. 또한, 귀납적 편향을 부여하지 않는 인공신경망 기반 모형은 높은 자유도로 인하여 과적합 가능성이 제기되어 왔다. 그러나 모델링 대상의 복잡도가 높은 경우에는 이 자유도에 기반하여 데이터로부터 관계 구조를 학습함으로써 희소한 환경에서도 우수한 예측력을 보일 수 있음을 확인하였다. 이상의 결과는 귀납적 편향에 적합한 변수 설계가 이루어진 경우 강건한 예측력을 기대할 수 있지만, 그렇지 못한 경우에는 데이터로부터 그 구조를 학습하는 컴포넌트를 도입하는 것이 효과적임을 시사한다.
...
교호작용 모델링 함수의 연산 과정을 구체적으로 살펴보면, 우선, 성립 가능한 2차 교호작용 쌍(Pair)에 대하여 차원별 곱셈(Element-wise Product)을 수행하여 동차원 교호작용 값을 도출한다(Outer Product). 이후 모든 쌍에 대한 `channels` 개의 선형 결합을 차원별로 도출한다($1\times 1$ Convoluation). 합성곱 계층의 수가 총 $N$ 장이라고 한다면, CIN은 총 $N+1$차 교호작용의 선형 결합을 도출하게 된다. 즉, 고차 상호작용 효과를 모델링함에 있어, DNN은 메커니즘을 관측 데이터로부터 학습하는 반면, CIN은 이를 요소별 곱셈으로 사전 규정한다는 점에서 차이가 있다.
...
[^1]: 선형성(Linearity)은 가법성(Additivity)과 동차성(Homogeneity)의 공리를 만족하는 성질이다. 여기서 가법성이란 기저들 간에 교섭이 발생하지 아니하여 특정 기저로 사상된 정보가 다른 기저들과 독립적으로 처리됨을 의미한다. 또한 동차성은 기저의 방향이 휘어지거나(Sigmoid) 차단되지(ReLU) 아니하여 특정 기저로 사상된 정보가 스케일의 변화에 의해 왜곡되지 아니하고 보존됨을 뜻한다. 종합하자면, 선형성이란 선형 공간의 기저, 즉 정보를 기술하는 의미 체계의 기본 단위들에 대한 가정으로서, 이들 단위 자체가 변형되지 아니하고 상호간에 의존하지 않아 독립적으로 유지되는 성질로 해석될 수 있다.

[^2]: 쌍선형 관계(Bi-Linear Relationship)란 두 변수 $\mathbf{u},\mathbf{v}$ 중 어느 하나를 고정하였을 때 다른 하나가 선형이 되는 관계를 말한다. 이 관계는 공간 $\mathbf{u}\in \mathcal{U}$ 와 $\mathbf{v}\in \mathcal{V}$ 가 모두 선형 공간이고, 서로 선형 변환으로 사상될 수 있다고 가정한다. 즉, $\mathcal{U}$ 의 기저는 $\mathcal{V}$ 의 기저들의 선형 결합으로 표현될 수 있으며, 반대로 $\mathcal{V}$ 의 기저 역시 $\mathcal{U}$ 의 기저들의 선형결합으로 표현될 수 있다. 이는 두 공간이 동일한 선형 구조를 공유함을 의미한다.

[^3]: 직관적으로 이해하자면, 쌍선형 관계 구조는 사용자를 구성하는 잠재 특징이 광고를 구성하는 잠재 특징들을 일정 비율로 조합한 결과로써 설명될 수 있음을 뜻한다. 다시 말해, 두 엔티티를 설명하는 요인들은 서로 무관하지 않고 하나의 공통된 의미 체계를 서로 다른 관점에서 기술한 것에 불과하다. 따라서 이 관계 구조 하에서 두 엔티티 간 교호작용 효과는 교호작용만을 설명하기 위한 새로운 의미 체계를 요청하지 않고서도, 이미 공유하고 있는 의미 체계 위에서 직접 평가될 수 있다.

[^4]: 쌍선형 교호작용 함수의 일반형 $f(\mathbf{u},\mathbf{v})=\mathbf{u}^{T}\mathbf{W}\mathbf{v}$ 에서 $\mathbf{W}$ 는 벡터 $\mathbf{v}$ 를 $\mathbf{u}$ 의 공간으로 사상하는 선형 변환 행렬이다. 내적(혹은 요소별 곱셈)에서 $\mathbf{W}$ 는 항등 행렬이 된다. 이는 두 변수가 사상되어 있는 선형 공간이 동일함을 전제한다. 직관적으로 이해하자면, 이 가정은 사용자와 광고, 그리고 이를 둘러싼 맥락들이 모두 동일한 특징들로써 설명될 수 있음을 의미한다.